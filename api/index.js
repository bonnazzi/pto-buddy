...
async function updateRequestStatus(userId, start, end, status) {
  ...
  try {
    ...
    const rowIndex = rows.findIndex(
      (r) => r[1] === userId && r[2] === start && r[3] === end && r[5] === "pending"
    );
  ...
  try {
    ...
    if (contentType.includes("application/x-www-form-urlencoded")) {
      ...
      if (body.payload) {
        ...
        if (payload.type === "block_actions") {
          ...
          if (actionId === "approve") {
            ...
            // <-- properly leave the deny branch
            res.statusCode = 200;
            res.end("");
            return;
          }
      ...
      if (body.command === "/pto" || body.command === "/pto-report") {
        ...
        // Original /pto command handling (direct; avoids undefined `context`)
        if (body.command === "/pto") {
          ...
          try {
            ...
            if (bal.remaining <= 0) {
              log.warn("User has insufficient PTO balance", {
                userId,
                balance: bal,
              });
            ...
            const daysRequested =
              Math.ceil(
                (requestEndDate - requestStartDate) / (1000 * 60 * 60 * 24)
              ) + 1;
